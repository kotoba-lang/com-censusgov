(ns censusgov.main
  "Kotodama WASM entrypoint for the Censusgov clean-room actor (L4) — Clojure port.
  ...
  facts (`censusgov.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "censusgov")
(def tier "L4")
...
(def entity-specs
  [{:entity "Filing"       :plural "filings"       :id-prefix "censusgo_fil"
    :fields [:filerId :form :period :status]       :required [:form :period]
    :coerce {}                       :refs {}}
   {:entity "Entity"       :plural "entities"      :id-prefix "censusgo_ent"
    :fields [:name :ein :jurisdiction]             :required [:name :ein]
    :coerce {}                       :refs {}}
   {:entity "Dataset"      :plural "datasets"      :id-prefix "censusgo_dat"
    :fields [:name :agency :updatedAt]             :required [:name :agency]
    :coerce {}                       :refs {}}
   {:entity "Application"  :plural "applications"  :id-prefix "censusgo_app"
    :fields [:applicantId :program :status]        :required [:program :status]
    :coerce {}                       :refs {}}
   {:entity "PublicRecord" :plural "publicrecords" :id-prefix "censusgo_pub"
    :fields [:recordType :jurisdiction :recordedAt] :required [:recordType :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Notice"       :plural "notices"       :id-prefix "censusgo_not"
...
(defn emit-facts
  "EAVT facts for one record: {\"censusgov.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "censusgov-compat" :tier tier :entities entities} 200])
...
(defn start! [] :censusgov-compat/ready)
